...
public enum MongoClientProvider {
    ...
    private String dbName = "Ecommerce-web";
    ...
    public MongoClient client(){
        if(client == null)throw new IllegalStateException("mongo client provider no inicializado, llama a init init antes");
        return client;
    }
    ...
    public<T> MongoCollection<T>getcCollection(String collectionName, Class<T>clazz){
        if(client == null)
            throw new IllegalStateException("mongo client provider no inicializado, llama a init  antes");
